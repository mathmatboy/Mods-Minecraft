...
public class BlockcommodeMini extends Block{

	public BlockcommodeMini(Material material)
	{
		super(material);
	}
	   public boolean hasTileEntity(int metadata)
	    ...
	    {
	        return new TileEntityBlockCommode();
	    }
	    ...
	    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase living, ItemStack stack)
	    {
	        if(stack.getItemDamage() == 0)
	        {
	            ...
	            if(tile instanceof TileEntityBlockCommode)
	            {
	                int direction = MathHelper.floor_double((double)(living.rotationYaw * 4.0F / 360.0F) + 2.5D) & 3;
	                ((TileEntityBlockCommode)tile).setDirection((byte)direction);
	            }
	        }
	    }
	    ...
	    public boolean rotateBlock(World world, int x, int y, int z, ForgeDirection axis)
	    {
	        if((axis == ForgeDirection.UP || axis == ForgeDirection.DOWN) && !world.isRemote && world.getBlockMetadata(x, y, z) == 0)
	        {
	            ...
	            if(tile instanceof TileEntityBlockCommode)
	            {
	            	TileEntityBlockCommode tileDirectional = (TileEntityBlockCommode)tile;
